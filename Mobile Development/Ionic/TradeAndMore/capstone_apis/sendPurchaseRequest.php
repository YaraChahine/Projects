...
$query = "SELECT item_price from items where item_id=?";
$stmt = $connection->prepare($query);
$stmt->bind_param("i",$receiver_item_id);
$stmt->execute();
$results = $stmt->get_result();
$item_price=$results->fetch_assoc();
$stmt->close();


$query = "INSERT INTO pending_requests(sender_user_id,receiver_user_id,receiver_item_id,requested_item_price,
request_msg) values (?,?,?,?,?);";
$stmt = $connection->prepare($query);
$stmt->bind_param("iiiis", $logged_in_id,$receiver_user_id,$receiver_item_id,$item_price, $text_message);
$stmt->execute();
$results = $stmt->get_result();
$rows=$text_message;
...
$stmt2->close();
mail($row2["email"],"Purchase Request","Dear ".$row["first_name_r"]." ".$row["last_name_r"].",\n\n".
$row["first_name_s"]." ".$row["last_name_s"]." has made a request to purchase your '".$row["item_name_r"]."'.",'From: tradeandmore.customerservice@gmail.com');
