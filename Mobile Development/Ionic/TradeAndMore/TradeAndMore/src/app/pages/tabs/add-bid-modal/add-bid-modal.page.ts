...
@Component({
  ...
  templateUrl: './add-bid-modal.page.html',
  styleUrls: ['./add-bid-modal.page.scss'],
})
export class AddBidModalPage implements OnInit {
  ...
  constructor(private modalCtrl: ModalController,private service: MyAPIsService, private http : HttpClient) {
...
public onSubmit(){
  ...
  this.http.post('http://localhost/capstone_apis/postImageResults.php',formData).subscribe((res:any)=>{
   ...
   if ( this.subcategories[subcat].subcategory_name==this.item_subcategory_name){
    ...
    this.service.addItemBid(data,this.user_id).subscribe((ress:any)=>{
