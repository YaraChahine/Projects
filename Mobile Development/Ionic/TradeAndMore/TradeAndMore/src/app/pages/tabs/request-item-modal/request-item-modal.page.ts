...
@Component({
  selector: 'app-request-item-modal',
  templateUrl: './request-item-modal.page.html',
  styleUrls: ['./request-item-modal.page.scss'],
})
export class RequestItemModalPage implements OnInit {
  ...
  constructor(public  toastController: ToastController,public _apiService: MyAPIsService,private modalCtrl: ModalController) {
...
this.item_result=0;
  ...
  sendRequest(){
    ...
    this._apiService.sendRequest(this.receiver_user_id,this.receiver_item_id,
      localStorage.getItem("logged-in-user-id"),this.selected_item.item_id,this.text_message).subscribe(async (res:any)=>{
